...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	private void setWindowIcon() {
		...
		if (util$enumos != Util.EnumOS.OSX) {
			...
			try {
				...
				if (inputstream != null && inputstream1 != null) Display.setIcon(new ByteBuffer[] { this.readImageToBuffer(inputstream), this.readImageToBuffer(inputstream1) });
	...
	private static boolean isJvm64bit() {
		...
		for (final String s : astring) { final String s1 = System.getProperty(s); if (s1 != null && s1.contains("64")) return true; }
		return false;
	}

	public Framebuffer getFramebuffer() { return this.framebufferMc; }

	public String getVersion() { return this.launchedVersion; }
	...
	public void refreshResources() {
		...
		for (final ResourcePackRepository.Entry resourcepackrepository$entry : this.mcResourcePackRepository.getRepositoryEntries()) list.add(resourcepackrepository$entry.getResourcePack());
		if (this.mcResourcePackRepository.getResourcePackInstance() != null) list.add(this.mcResourcePackRepository.getResourcePackInstance());
		try {
	...
	private void updateDisplayMode() throws LWJGLException {
		final Set<DisplayMode> set = Sets.<DisplayMode>newHashSet();
		Collections.addAll(set, Display.getAvailableDisplayModes());
		DisplayMode displaymode = Display.getDesktopDisplayMode();
		if (!set.contains(displaymode) && Util.getOSType() == Util.EnumOS.OSX) label53: for (final DisplayMode displaymode1 : macDisplayModes) {
			boolean flag = true;
			for (final DisplayMode displaymode2 : set) if (displaymode2.getBitsPerPixel() == 32 && displaymode2.getWidth() == displaymode1.getWidth() && displaymode2.getHeight() == displaymode1.getHeight()) {
				flag = false;
				break;
			}
			if (!flag) {
				...
				while (true) {
					...
					if (displaymode3.getBitsPerPixel() == 32 && displaymode3.getWidth() == displaymode1.getWidth() / 2 && displaymode3.getHeight() == displaymode1.getHeight() / 2) break;
				}
				displaymode = displaymode3;
			}
		}
	 ...
	 */
	public ISaveFormat getSaveLoader() { return this.saveLoader; }
	...
	private void runGameLoop() throws IOException {
		...
		while (getSystemTime() >= this.debugUpdateTime + 1000L) {
			...
			this.debug = String.format("%d fps (%d chunk update%s) T: %s%s%s%s%s", Integer.valueOf(debugFPS), Integer.valueOf(RenderChunk.renderChunksUpdated), RenderChunk.renderChunksUpdated != 1 ? "s" : "",
	...
	public int getLimitFramerate() { return this.theWorld == null && this.currentScreen != null ? Settings.NO_FPS_LIMIT_IN_GUIS ? Integer.MAX_VALUE : 30 : this.gameSettings.limitFramerate; }

	public boolean isFramerateLimitBelowMax() { return this.getLimitFramerate() < GameSettings.Options.FRAMERATE_LIMIT.getValueMax(); }
	...
	private void displayDebugInfo(final long elapsedTicksTime) {
		if (this.mcProfiler.profilingEnabled) {
			...
			for (final Result element : list) {
				final Profiler.Result profiler$result1 = element;
				...
				for (int j2 = i1; j2 >= 0; --j2) {
					final float f = (float) ((d0 + profiler$result1.field_76332_a * j2 / i1) * Math.PI * 2.0D / 100.0D);
				...
				for (int i3 = i1; i3 >= 0; --i3) {
					final float f3 = (float) ((d0 + profiler$result1.field_76332_a * i3 / i1) * Math.PI * 2.0D / 100.0D);
	...
	private void sendClickBlockToController(final boolean leftClick) {
		...
		if (this.leftClickCounter <= 0 && !this.thePlayer.isUsingItem()) if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
	...
	private void rightClickMouse() {
		if (!this.playerController.getIsHittingBlock()) {
			...
			case ENTITY:
				if (this.playerController.isPlayerRightClickingOnEntity(this.thePlayer, this.objectMouseOver.entityHit, this.objectMouseOver)) flag = false;
			...
			if (flag) {
				...
				if (itemstack1 != null && this.playerController.sendUseItem(this.thePlayer, this.theWorld, itemstack1)) this.entityRenderer.itemRenderer.resetEquippedProgress2();
			}
		}
	}
	...
	private void updateFramebufferSize() {
		...
		if (this.entityRenderer != null) this.entityRenderer.updateShaderGroupSize(this.displayWidth, this.displayHeight);
	}

	/**
	 * Return the musicTicker's instance
	 */
	public MusicTicker getMusicTicker() { return this.mcMusicTicker; }
	...
	public void runTick() throws IOException {
		...
		if (this.currentScreen == null && this.thePlayer != null) {
			if (this.thePlayer.getHealth() <= 0.0F) this.displayGuiScreen((GuiScreen) null);
			else if (this.thePlayer.isPlayerSleeping() && this.theWorld != null) this.displayGuiScreen(new GuiSleepMP());
		} else if (this.currentScreen != null && this.currentScreen instanceof GuiSleepMP && !this.thePlayer.isPlayerSleeping()) this.displayGuiScreen((GuiScreen) null);
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
			...
			while (Mouse.next()) {
				...
				if (Mouse.getEventButtonState()) if (this.thePlayer.isSpectator() && i == 2) this.ingameGUI.getSpectatorGui().func_175261_b();
				...
				if (i1 <= 200L) {
					...
					if (j != 0) if (this.thePlayer.isSpectator()) {
						j = j < 0 ? -1 : 1;
						if (this.ingameGUI.getSpectatorGui().func_175262_a()) this.ingameGUI.getSpectatorGui().func_175259_b(-j);
						else {
			...
			while (Keyboard.next()) {
				...
				} else if (Keyboard.isKeyDown(46) && Keyboard.isKeyDown(61)) this.debugCrashKeyPressTime = getSystemTime();
				...
				if (Keyboard.getEventKeyState()) {
					...
					if (this.currentScreen != null) this.currentScreen.handleKeyboardInput();
						...
						if (k == 32 && Keyboard.isKeyDown(61) && this.ingameGUI != null) this.ingameGUI.getChatGUI().clearChatMessages();
						...
						if (k == 33 && Keyboard.isKeyDown(61)) this.gameSettings.setOptionValue(GameSettings.Options.RENDER_DISTANCE, GuiScreen.isShiftKeyDown() ? -1 : 1);
						...
						if (this.gameSettings.keyBindTogglePerspective.isPressed()) {
							...
							if (this.gameSettings.thirdPersonView == 0) this.entityRenderer.loadEntityShader(this.getRenderViewEntity());
							else if (this.gameSettings.thirdPersonView == 1) this.entityRenderer.loadEntityShader((Entity) null);
							this.renderGlobal.setDisplayListEntitiesDirty();
						}
						if (this.gameSettings.keyBindSmoothCamera.isPressed()) this.gameSettings.smoothCamera = !this.gameSettings.smoothCamera;
					}
			...
			for (int l = 0; l < 9; ++l) if (this.gameSettings.keyBindsHotbar[l].isPressed()) if (this.thePlayer.isSpectator()) this.ingameGUI.getSpectatorGui().func_175260_a(l);
			...
			while (this.gameSettings.keyBindInventory.isPressed()) if (this.playerController.isRidingHorse()) this.thePlayer.sendHorseInventory();
			else {
				this.getNetHandler().addToSendQueue(new C16PacketClientStatus(C16PacketClientStatus.EnumState.OPEN_INVENTORY_ACHIEVEMENT));
				this.displayGuiScreen(new GuiInventory(this.thePlayer));
			}
			while (this.gameSettings.keyBindDrop.isPressed()) if (!this.thePlayer.isSpectator()) this.thePlayer.dropOneItem(GuiScreen.isCtrlKeyDown());
			while (this.gameSettings.keyBindChat.isPressed() && flag) this.displayGuiScreen(new GuiChat());
			if (this.currentScreen == null && this.gameSettings.keyBindCommand.isPressed() && flag) this.displayGuiScreen(new GuiChat("/"));
			if (this.thePlayer.isUsingItem()) {
				if (!this.gameSettings.keyBindUseItem.isKeyDown()) this.playerController.onStoppedUsingItem(this.thePlayer);
			...
			if (this.gameSettings.keyBindUseItem.isKeyDown() && this.rightClickDelayTimer == 0 && !this.thePlayer.isUsingItem()) this.rightClickMouse();
		...
		if (this.theWorld != null) {
			...
			if (!this.isGamePaused) {
				if (this.theWorld.getLastLightningBolt() > 0) this.theWorld.setLastLightningBolt(this.theWorld.getLastLightningBolt() - 1);
		...
		if (this.theWorld != null) {
			...
			if (!this.isGamePaused && this.theWorld != null) this.theWorld.doVoidFogParticles(MathHelper.floor_double(this.thePlayer.posX), MathHelper.floor_double(this.thePlayer.posY), MathHelper.floor_double(this.thePlayer.posZ));
		...
		// boolean fullScreenNow = isFullScreen();
	...
	public void fix(final boolean fullscreen) {
		try {
			if (fullscreen) {
	 ...
	 */
	public void loadWorld(final WorldClient worldClientIn) { this.loadWorld(worldClientIn, ""); }
	...
	public void setDimensionAndSpawnPlayer(final int dimension) {
		...
		if (this.currentScreen instanceof GuiGameOver) this.displayGuiScreen((GuiScreen) null);
	}

	/**
	 * Gets whether this is a demo or not.
	 */
	public final boolean isDemo() { return this.isDemo; }

	public NetHandlerPlayClient getNetHandler() { return this.thePlayer != null ? this.thePlayer.sendQueue : null; }

	public static boolean isGuiEnabled() { return theMinecraft == null || !theMinecraft.gameSettings.hideGUI; }

	public static boolean isFancyGraphicsEnabled() { return theMinecraft != null && theMinecraft.gameSettings.fancyGraphics; }

	/**
	 * Returns if ambient occlusion is enabled
	 */
	public static boolean isAmbientOcclusionEnabled() { return theMinecraft != null && theMinecraft.gameSettings.ambientOcclusion != 0; }
	...
	private void middleClickMouse() {
		if (this.objectMouseOver != null) {
			...
			if (this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
				...
				if (this.objectMouseOver.typeOfHit != MovingObjectPosition.MovingObjectType.ENTITY || this.objectMouseOver.entityHit == null || !flag) return;
	...
	public CrashReport addGraphicsAndWorldToCrashReport(final CrashReport theCrash) {
		...
		theCrash.getCategory().addCrashSectionCallable("Resource Packs", () -> {
			...
			for (final String s : Minecraft.this.gameSettings.resourcePacks) {
				...
				if (Minecraft.this.gameSettings.incompatibleResourcePacks.contains(s)) stringbuilder.append(" (incompatible)");
			}
	 ...
	 */
	public static Minecraft getMinecraft() { return theMinecraft; }

	public ListenableFuture<Object> scheduleResourcesRefresh() { return this.addScheduledTask(Minecraft.this::refreshResources); }
	...
	public void addServerStatsToSnooper(final PlayerUsageSnooper playerSnooper) {
		...
		int i = 0;
		for (final ResourcePackRepository.Entry resourcepackrepository$entry : this.mcResourcePackRepository.getRepositoryEntries()) playerSnooper.addClientStat("resource_pack[" + i++ + "]", resourcepackrepository$entry.getResourcePackName());
		if (this.theIntegratedServer != null && this.theIntegratedServer.getPlayerUsageSnooper() != null) playerSnooper.addClientStat("snooper_partner", this.theIntegratedServer.getPlayerUsageSnooper().getUniqueID());
	}
	...
	public static int getGLMaximumTextureSize() {
		for (int i = 16384; i > 0; i >>= 1) {
			...
			GL11.glTexImage2D(GL11.GL_PROXY_TEXTURE_2D, 0, GL11.GL_RGBA, i, i, 0, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, buffer);
	 ...
	 */
	@Override
	public boolean isSnooperEnabled() { return this.gameSettings.snooperEnabled; }

	/**
	 * Set the current ServerData instance.
	 */
	public void setServerData(final ServerData serverDataIn) { this.currentServerData = serverDataIn; }

	public ServerData getCurrentServerData() { return this.currentServerData; }

	public boolean isIntegratedServerRunning() { return this.integratedServerIsRunning; }

	/**
	 * Returns true if there is only one player playing, and the current server is the integrated one.
	 */
	public boolean isSingleplayer() { return this.integratedServerIsRunning && this.theIntegratedServer != null; }

	/**
	 * Returns the currently running integrated server
	 */
	public IntegratedServer getIntegratedServer() { return this.theIntegratedServer; }
	 ...
	 */
	public static long getSystemTime() { return Sys.getTime() * 1000L / Sys.getTimerResolution(); }
	...
	public PropertyMap getTwitchDetails() { return this.twitchDetails; }
	...
	public TextureManager getTextureManager() { return this.renderEngine; }

	public IResourceManager getResourceManager() { return this.mcResourceManager; }

	public ResourcePackRepository getResourcePackRepository() { return this.mcResourcePackRepository; }

	public LanguageManager getLanguageManager() { return this.mcLanguageManager; }

	public TextureMap getTextureMapBlocks() { return this.textureMapBlocks; }

	public boolean isJava64bit() { return this.jvm64bit; }

	public boolean isGamePaused() { return this.isGamePaused; }

	public SoundHandler getSoundHandler() { return this.mcSoundHandler; }
	...
	public void dispatchKeypresses() {
		...
		if (i != 0 && !Keyboard.isRepeatEvent()) if (!(this.currentScreen instanceof GuiControls) || ((GuiControls) this.currentScreen).time <= getSystemTime() - 20L) if (Keyboard.getEventKeyState()) {
			if (i == this.gameSettings.keyBindStreamStartStop.getKeyCode()) {
				if (this.getTwitchStream().isBroadcasting()) this.getTwitchStream().stopBroadcasting();
				else if (this.getTwitchStream().isReadyToBroadcast())
					this.displayGuiScreen(new GuiYesNo((result, id) -> { if (result) Minecraft.this.getTwitchStream().func_152930_t(); Minecraft.this.displayGuiScreen((GuiScreen) null); }, I18n.format("stream.confirm_start"), "", 0));
				else if (this.getTwitchStream().func_152928_D() && this.getTwitchStream().func_152936_l()) {
					if (this.theWorld != null) this.ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("Not ready to start streaming yet!"));
				...
				if (this.getTwitchStream().isBroadcasting()) if (this.getTwitchStream().isPaused()) this.getTwitchStream().unpause();
				else this.getTwitchStream().pause();
			...
			} else if (i == this.gameSettings.keyBindStreamToggleMic.getKeyCode()) this.stream.muteMicrophone(true);
		...
		} else if (i == this.gameSettings.keyBindStreamToggleMic.getKeyCode()) this.stream.muteMicrophone(false);
	}

	public MinecraftSessionService getSessionService() { return this.sessionService; }

	public SkinManager getSkinManager() { return this.skinManager; }

	public Entity getRenderViewEntity() { return this.renderViewEntity; }
	...
	@Override
	public boolean isCallingFromMinecraftThread() { return Thread.currentThread() == this.mcThread; }

	public BlockRendererDispatcher getBlockRendererDispatcher() { return this.blockRenderDispatcher; }

	public RenderManager getRenderManager() { return this.renderManager; }

	public RenderItem getRenderItem() { return this.renderItem; }

	public ItemRenderer getItemRenderer() { return this.itemRenderer; }

	public static int getDebugFPS() { return debugFPS; }

	/**
	 * Return the FrameTimer's instance
	 */
	public FrameTimer getFrameTimer() { return this.frameTimer; }
	 ...
	 */
	public boolean isConnectedToRealms() { return this.connectedToRealms; }
	 ...
	 */
	public void setConnectedToRealms(final boolean isConnected) { this.connectedToRealms = isConnected; }
}
