...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(final float partialTicks) {
		...
		if (this.mc.gameSettings.thirdPersonView == 0 && itemstack != null && itemstack.getItem() == Item.getItemFromBlock(Blocks.pumpkin)) this.renderPumpkinOverlay(scaledresolution);
		...
		if (this.mc.gameSettings.heldItemTooltips && !this.mc.playerController.isSpectator()) this.renderSelectedItem(scaledresolution);
	...
	protected void renderTooltip(final ScaledResolution sr, final float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			for (int j = 0; j < 9; ++j) { final int k = sr.getScaledWidth() / 2 - 90 + j * 20 + 2; final int l = sr.getScaledHeight() - 16 - 3; this.renderHotbarItem(j, k, l, partialTicks, entityplayer); }
	...
	protected boolean showCrosshair() {
		if (this.mc.gameSettings.showDebugInfo && !this.mc.thePlayer.hasReducedDebug() && !this.mc.gameSettings.reducedDebugInfo) return false;
			...
			if (this.mc.pointedEntity != null) return true;
				...
				if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
					...
					if (this.mc.theWorld.getTileEntity(blockpos) instanceof IInventory) return true;
				}
				return false;
			}
		} else return true;
	...
	public void renderStreamIndicator(final ScaledResolution scaledRes) { this.streamIndicator.render(scaledRes.getScaledWidth() - 10, 10); }
	...
	private void renderScoreboard(final ScoreObjective objective, final ScaledResolution scaledRes) {
		...
		final List<Score> list = Lists.newArrayList(Iterables.filter(collection, p_apply_1_ -> p_apply_1_.getPlayerName() != null && !p_apply_1_.getPlayerName().startsWith("#")));
	...
	private void renderPlayerStats(final ScaledResolution scaledRes) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			int l2 = -1;
			if (entityplayer.isPotionActive(Potion.regeneration)) l2 = this.updateCounter % MathHelper.ceiling_float_int(f + 5.0F);
			this.mc.mcProfiler.startSection("armor");
			for (int i3 = 0; i3 < 10; ++i3) if (k2 > 0) {
			...
			if (entity == null) {
				...
				for (int k6 = 0; k6 < 10; ++k6) {
					...
					if (entityplayer.getFoodStats().getSaturationLevel() <= 0.0F && this.updateCounter % (k * 3 + 1) == 0) j7 = k1 + (this.rand.nextInt(3) - 1);
					if (flag1) k8 = 1;
			...
			if (entityplayer.isInsideOfMaterial(Material.water)) {
				...
				for (int l8 = 0; l8 < k7 + i8; ++l8) if (l8 < k7) this.drawTexturedModalRect(j1 - l8 * 8 - 9, j2, 16, 18, 9, 9);
	...
	private void renderVignette(float lightLevel, final ScaledResolution scaledRes) {
		if (!Config.isVignetteEnabled()) {
			...
			else GlStateManager.color(this.prevVignetteBrightness, this.prevVignetteBrightness, this.prevVignetteBrightness, 1.0F);
	...
	public void setRecordPlayingMessage(final String recordName) { this.setRecordPlaying(I18n.format("record.nowPlaying", recordName), true); }
	...
	public void setRecordPlaying(final IChatComponent component, final boolean isPlaying) { this.setRecordPlaying(component.getUnformattedText(), isPlaying); }

	/**
	 * returns a pointer to the persistant Chat GUI, containing all previous chat messages and such
	 */
	public GuiNewChat getChatGUI() { return this.persistantChatGUI; }

	public int getUpdateCounter() { return this.updateCounter; }

	public FontRenderer getFontRenderer() { return this.mc.fontRendererObj; }

	public GuiSpectator getSpectatorGui() { return this.spectatorGui; }

	public GuiPlayerTabOverlay getTabList() { return this.overlayPlayerList; }

	/**
	 * Reset the GuiPlayerTabOverlay's message header and footer
	 */
	public void resetPlayersOverlayFooterHeader() { this.overlayPlayerList.resetFooterHeader(); }
}
