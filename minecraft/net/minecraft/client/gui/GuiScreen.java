...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
	...
	protected void renderToolTip(final ItemStack stack, final int x, final int y) {
		...
		for (int i = 0; i < list.size(); ++i) if (i == 0) list.set(i, stack.getRarity().rarityColor + list.get(i));
	 ...
	 */
	protected void drawCreativeTabHoveringText(final String tabName, final int mouseX, final int mouseY) { this.drawHoveringText(Arrays.<String>asList(tabName), mouseX, mouseY); }
	...
	protected void drawHoveringText(final List<String> textLines, final int x, final int y) {
		if (!textLines.isEmpty()) {
			...
			int i = 0;
			for (final String s : textLines) { final int j = this.fontRendererObj.getStringWidth(s); if (j > i) i = j; }
			int l1 = x + 12;
			int i2 = y - 12;
			int k = 8;
			...
			for (int k1 = 0; k1 < textLines.size(); ++k1) { final String s1 = textLines.get(k1); this.fontRendererObj.drawStringWithShadow(s1, l1, i2, -1); if (k1 == 0) i2 += 2; i2 += 10; }
			this.zLevel = 0.0F;
	...
	protected void handleComponentHover(final IChatComponent component, final int x, final int y) {
		if (component != null && component.getChatStyle().getChatHoverEvent() != null) {
			...
			if (hoverevent.getAction() == HoverEvent.Action.SHOW_ITEM) {
				...
				if (this.mc.gameSettings.advancedItemTooltips) try {
					...
					if (nbtbase1 instanceof NBTTagCompound) {
						final List<String> list1 = Lists.<String>newArrayList();
			...
			} else if (hoverevent.getAction() == HoverEvent.Action.SHOW_TEXT) this.drawHoveringText(NEWLINE_SPLITTER.splitToList(hoverevent.getValue().getFormattedText()), x, y);
				...
				if (statbase != null) {
					...
					this.drawHoveringText(list, x, y);
				} else this.drawCreativeTabHoveringText(EnumChatFormatting.RED + "Invalid statistic/achievement!", x, y);
			}
			GlStateManager.disableLighting();
		}
	}
	...
	protected boolean handleComponentClick(final IChatComponent component) {
		if (component == null) return false;
			...
			if (isShiftKeyDown()) {
				if (component.getChatStyle().getInsertion() != null) this.setText(component.getChatStyle().getInsertion(), false);
				...
				} else if (clickevent.getAction() == ClickEvent.Action.SUGGEST_COMMAND) this.setText(clickevent.getValue(), true);
				else if (clickevent.getAction() == ClickEvent.Action.RUN_COMMAND) this.sendChatMessage(clickevent.getValue(), false);
				else if (clickevent.getAction() == ClickEvent.Action.TWITCH_USER_INFO) {
					final ChatUserInfo chatuserinfo = this.mc.getTwitchStream().func_152926_a(clickevent.getValue());
					if (chatuserinfo != null) this.mc.displayGuiScreen(new GuiTwitchUserMode(this.mc.getTwitchStream(), chatuserinfo));
	 ...
	 */
	public void sendChatMessage(final String msg) { this.sendChatMessage(msg, true); }
	 ...
	 * background.png
	 */
	public void drawDefaultBackground() { this.drawWorldBackground(0); }
	 ...
	 */
	public boolean doesGuiPauseGame() { return Settings.NETHER_PORTAL_GUI_FIX; }
	...
	private void openWebLink(final URI url) {
		try {
			...
			final Object object = oclass.getMethod("getDesktop").invoke((Object) null);
	...
	public static boolean isKeyComboCtrlX(final int keyID) { return keyID == 45 && isCtrlKeyDown() && !isShiftKeyDown() && !isAltKeyDown(); }

	public static boolean isKeyComboCtrlV(final int keyID) { return keyID == 47 && isCtrlKeyDown() && !isShiftKeyDown() && !isAltKeyDown(); }

	public static boolean isKeyComboCtrlC(final int keyID) { return keyID == 46 && isCtrlKeyDown() && !isShiftKeyDown() && !isAltKeyDown(); }

	public static boolean isKeyComboCtrlA(final int keyID) { return keyID == 30 && isCtrlKeyDown() && !isShiftKeyDown() && !isAltKeyDown(); }
	 ...
	 * @param h The height of the screen
	 */
	public void onResize(final Minecraft mcIn, final int w, final int h) { this.setWorldAndResolution(mcIn, w, h); }
}
