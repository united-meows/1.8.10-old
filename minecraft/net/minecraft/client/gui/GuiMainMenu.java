...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	public GuiMainMenu()
	{
		...
		try {
			...
			String s;
			while ((s = bufferedreader.readLine()) != null) { s = s.trim(); if (!s.isEmpty()) list.add(s); }
			if (!list.isEmpty()) while (true) {
				this.splashText = list.get(RANDOM.nextInt(list.size()));
				if (this.splashText.hashCode() == 125780783) {
					this.splashText = "Mojang removed this splash.";
					break;
				} else if (this.splashText.hashCode() != 125780783) break;
			}
		} catch (final IOException var12) {} finally {
	...
	public void initGui() {
		...
		if (calendar.get(2) + 1 == 12 && calendar.get(5) == 24) this.splashText = "Merry X-mas!";
		else if (calendar.get(2) + 1 == 1 && calendar.get(5) == 1) this.splashText = "Happy new year!";
		else if (calendar.get(2) + 1 == 10 && calendar.get(5) == 31) this.splashText = "OOoooOOOoooo! Spooky!";
		final int i = 24;
	...
	protected void actionPerformed(final GuiButton button) throws IOException {
		...
		if (button.id == 6 && Reflector.GuiModList_Constructor.exists()) this.mc.displayGuiScreen((GuiScreen) Reflector.newInstance(Reflector.GuiModList_Constructor, this));
	...
	private void renderSkybox(final int p_73971_1_, final int p_73971_2_, final float p_73971_3_) {
		...
		for (int j = 0; j < i; ++j) { this.rotateAndBlurSkybox(p_73971_3_); this.rotateAndBlurSkybox(p_73971_3_); }
	...
	public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
		...
		GlStateManager.popMatrix();
		String s = MinecraftInstance.INSTANCE.NAME;
		if (this.mc.isDemo()) s = s + " Demo";
		if (Reflector.FMLCommonHandler_getBrandings.exists()) {
			final Object object = Reflector.call(Reflector.FMLCommonHandler_instance);
			final List<String> list = Lists.<String>reverse((List) Reflector.call(object, Reflector.FMLCommonHandler_getBrandings, Boolean.valueOf(true)));
