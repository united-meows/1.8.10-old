...
public class DynamicLights {
		...
		if (i >= timeUpdateMs + 50L) {
			...
			synchronized (mapDynamicLights) {
				...
				if (mapDynamicLights.size() > 0) {
					...
					for (int j = 0; j < list.size(); ++j) { final DynamicLight dynamiclight = list.get(j); dynamiclight.update(renderGlobal); }
				}
			}
		}
	}
	...
	public static void removeLights(final RenderGlobal renderGlobal) {
		synchronized (mapDynamicLights) {
			final List<DynamicLight> list = mapDynamicLights.valueList();
			for (int i = 0; i < list.size(); ++i) { final DynamicLight dynamiclight = list.get(i); dynamiclight.updateLitChunks(renderGlobal); }
