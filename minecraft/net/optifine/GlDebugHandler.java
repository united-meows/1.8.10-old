...
public class GlDebugHandler implements Handler {
	...
	public void handleMessage(final int source, final int type, final int id, final int severity, final String message) {
		if (!message.contains("glBindFramebuffer") && !message.contains("Wide lines")) {
	...
	private String getUnknown(final int token) { return "Unknown (0x" + Integer.toHexString(token).toUpperCase() + ")"; }
}
